...
def apply_trend_override(data, product, override_type, custom_growth_rate=0.0):


    """


    Apply trend override to forecast data for a specific product.


    


    This function modifies forecast values to override statistical declining trends


    while preserving seasonal patterns.
    ...
    Args:
        ...
        data: DataFrame with forecast data (must have Date, Product, Type, ACR columns)


        product: Product name to apply override to


        override_type: One of 'none', 'flat', 'last_growth', 'custom'


        custom_growth_rate: Monthly growth rate to apply (only used if override_type='custom')
    ...
    Returns:
        ...
        DataFrame with modified forecast values for the specified product


    """


    if override_type == 'none' or data.empty:
    ...
    product_mask = result['Product'] == product


    product_data = result[product_mask].sort_values('Date')
    ...
    if 'Type' in product_data.columns:


        actual_mask = product_data['Type'].isin(['actual', 'history', 'historical'])


        forecast_mask = product_data['Type'] == 'forecast'


    else:


        return result  # Can't determine without Type column


    


    actuals = product_data[actual_mask].sort_values('Date')


    forecasts = product_data[forecast_mask].sort_values('Date')
    ...
    last_actual_value = actuals['ACR'].iloc[-1]


    


    # Calculate seasonal indices from historical data (last 12 months if available)
    ...
    if len(actuals) >= 12:
        ...
        last_12 = actuals.tail(12).copy()


        last_12['month'] = pd.to_datetime(last_12['Date']).dt.month


        monthly_avg = last_12.groupby('month')['ACR'].mean()
    ...
    # Apply the trend override


    forecast_indices = forecasts.index.tolist()


    forecast_dates = pd.to_datetime(forecasts['Date']).tolist()


    


    if override_type == 'flat':


        # Flat trend: maintain last actual value with seasonality


        base_value = last_actual_value


        for i, (idx, date) in enumerate(zip(forecast_indices, forecast_dates)):
            ...
            if seasonal_indices:
                ...
                result.loc[idx, 'ACR'] = base_value * seasonal_factor


            else:


                result.loc[idx, 'ACR'] = base_value


                


    elif override_type == 'last_growth':


        # Maintain the average growth rate from last 6 months of actuals


        if len(actuals) >= 6:


            recent_actuals = actuals.tail(6)['ACR'].values


            mom_changes = np.diff(recent_actuals) / recent_actuals[:-1]


            mom_changes = mom_changes[np.isfinite(mom_changes)]


            if len(mom_changes) > 0:


                avg_growth = np.mean(mom_changes)


                # Ensure non-negative growth (flat minimum)


                avg_growth = max(0, avg_growth)
        ...
        for i, (idx, date) in enumerate(zip(forecast_indices, forecast_dates)):
            ...
            base_value = prev_value * (1 + avg_growth)
    ...
    elif override_type == 'custom':
        # Apply custom monthly growth rate (pure compounding, no seasonality)
        monthly_growth = custom_growth_rate / 100.0  # Convert percentage to decimal
        prev_value = last_actual_value
        for i, (idx, date) in enumerate(zip(forecast_indices, forecast_dates)):
            base_value = prev_value * (1 + monthly_growth)
            # For custom growth: pure compounding without seasonal adjustment
            result.loc[idx, 'ACR'] = base_value
            prev_value = base_value
...
def display_product_forecast(data, product, model_name, best_models_per_product=None, best_mapes_per_product=None):
    ...
    with col1:
        ...
        if 'BestModel' in product_data.columns and not product_data['BestModel'].isna().all():
            ...
            if reason:
                ...
                st.caption(f"📊 Using **{actual_model}** model for this product â€” {reason}")
    ...
    with col2:
        ...
        # Prefer perâ€‘model perâ€‘product WAPE from session, fallback to best_mapes_per_product
    ...
    try:
        ...
        # Prepare backtesting data using the winning perâ€‘product model
...
"""
        ...
        if has_noncompliant_historical:
            ...
            legend_text += "            - ï¿½ **Red Dotted**: Historical non-compliant rev rec renewals\n"


        if has_noncompliant_forecast:


            legend_text += "            - ï¿½ **Dark Red Dashed**: Future non-compliant rev rec renewals\n"
...
"""
        ...
        st.error(f"âŒ Chart generation failed: {str(e)}")
...
def prepare_backtesting_chart_data(backtesting_results, product_name, model_name):
    ...
    # If enhanced rolling provided perâ€‘fold series, assemble all folds; else show most recent only
...
def display_diagnostic_messages(messages, max_messages=10):
    ...
    error_msgs = [msg for msg in messages if msg.startswith("âŒ")]


    info_msgs = [msg for msg in messages if not any(msg.startswith(prefix) for prefix in ["✅", "⚠️", "âŒ"])]
    ...
    if error_msgs:
        ...
        with st.expander(f"âŒ Errors ({len(error_msgs)})", expanded=True):
...
def display_forecast_results():
    ...
    try:
        ...
        # Use mean of selected perâ€‘product WAPEs (backtesting view) if available
    ...
    label_explanations = {"Backtesting": "Deeper walkâ€‘forward / CV driven perâ€‘product picks (more robust)."}
    ...
    def _coerce_model_data(model_data):
        ...
        Handles cases where model_data is already a DataFrame or a list of perâ€‘product DataFrames.
    ...
    with metrics_cols[5]:
        ...
        st.metric("Avg MoM", f"{avg_mom:.1f}%" if np.isfinite(avg_mom) else "â€”", help="Average month-over-month growth")
    ...
    st.caption("Selection: lowest mean WAPE on backtesting (ties â†’ p75 WAPE â†’ MASE).")
    ...
    with st.expander("🔄 Products (selected mode)", expanded=False):
        ...
        if not df_active.empty:
            ...
            for product, grp in df_active[df_active['Type'].eq('forecast') if 'Type' in df_active.columns else df_active.index.isin(df_active.index)].groupby('Product'):
                ...
                trend = "â†‘" if np.isfinite(avg_mom_p) and avg_mom_p > 0.5 else ("â†“" if np.isfinite(avg_mom_p) and avg_mom_p < -0.5 else "â†’")


                # per product WAPE (prefer backtesting perâ€‘product mapes)
        ...
        if rows:
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    'WAPE%': st.column_config.NumberColumn('WAPE %', format="%.1f%%", help='Perâ€‘product validation WAPE (lower is better)'),
    ...
    if best_models_per_product:
        ...
        if st.session_state.get('model_avg_ranks'):
            ...
            st.success("✅ **Backtesting-Only Selection:** Each product uses the model with the lowest WAPE on crossâ€‘validated backtests (ties â†’ p75 WAPE â†’ MASE).")
            ...
            with st.expander("📊 How Selection Works (Backtesting WAPE)", expanded=False):
                ...
                **Perâ€‘product selection, scored strictly on backtesting WAPE:**


                - **Primary:** Recencyâ€‘weighted mean WAPE across folds (falls back to mean WAPE)


                - **Tieâ€‘breaks:** p75 WAPE â†’ MASE â†’ trendâ€‘improvement check


                - **Eligibility:** Enough history for â‰¥4 folds (current config â‰ˆ 30 months), MASE < 1.0, â‰¥10% better WAPE than Seasonalâ€‘Naive, and stability p95 WAPE â‰¤ 2.25Ã— mean (â‰¤2.5Ã— with high fold consistency; stricter thresholds for LightGBM).


                - **Fallback:** If backtesting is insufficient, the app falls back to **Best per Product (Standard)** multiâ€‘metric selection. Seasonalâ€‘Naive remains the baseline and may be chosen when it is the only eligible option.
    ...
    if not fy_data.empty:
        ...
        if not multi_year_data.empty:
            ...
            if fy_summary_rows:
                ...
                st.caption(
                    ...
                    f"Fiscal years displayed: {', '.join(selected_fys) if selected_fys else 'All'} â€¢ "
        ...
        st.caption(
            ...
            f"â° {months_elapsed} months elapsed, {months_remaining} months remaining"
    ...
    # === TREND OVERRIDE CONTROLS ===


    # Allow users to override declining statistical trends while preserving seasonality


    with st.expander("📈 **Trend Override** (Override declining forecast trends)", expanded=False):


        st.markdown("""


        **Override statistical declining trends** while preserving seasonal patterns.


        Use this when you believe the underlying business trend should be maintained despite 


        what the statistical model predicts.
        ...
        # Initialize session state for trend overrides if not exists


        if 'trend_overrides' not in st.session_state:


            st.session_state.trend_overrides = {}


        


        # Get current override for this product


        current_override = st.session_state.trend_overrides.get(product, {'type': 'none', 'custom_rate': 0.0})
        ...
        with col_override1:
            ...
            override_options = {
                ...
                'none': '📊 Statistical Model (Default)',


                'flat': '➡️ Flat Trend (Maintain Last Value)',


                'last_growth': '📈 Continue Recent Growth (Avg of Last 6 Months)',


                'custom': '🎯 Custom Monthly Growth Rate'
            ...
            override_type = st.selectbox(
                ...
                f"Trend Override for {product}",


                options=list(override_options.keys()),


                format_func=lambda x: override_options[x],


                index=list(override_options.keys()).index(current_override['type']),


                key=f"trend_override_{product}",


                help="Override the statistical forecast trend while preserving seasonal fluctuations"
        ...
        with col_override2:
            ...
            if override_type == 'custom':
                ...
                custom_growth_rate = st.number_input(
                    ...
                    value=float(current_override.get('custom_rate', 0.0)),


                    step=0.5,


                    key=f"custom_growth_{product}",


                    help="Monthly growth rate to apply (e.g. 2.0 = 2% monthly growth)"
        ...
        new_override = {'type': override_type, 'custom_rate': custom_growth_rate}


        if new_override != current_override:


            st.session_state.trend_overrides[product] = new_override
        ...
        if override_type == 'flat':
            ...
            st.info("✅ **Flat Trend**: Forecast will maintain the last actual value with seasonal adjustments.")


        elif override_type == 'last_growth':


            st.info("📈 **Continue Growth**: Forecast will continue the average growth rate from the last 6 months.")


        elif override_type == 'custom':


            annualized = (1 + custom_growth_rate/100)**12 - 1


            st.info(f"🎯 **Custom Growth**: {custom_growth_rate:.1f}% monthly = {annualized*100:.1f}% annualized")


    


    # Apply trend override if selected


    display_data = chart_model_data.copy()


    current_product_override = st.session_state.trend_overrides.get(product, {'type': 'none', 'custom_rate': 0.0})


    if current_product_override['type'] != 'none':


        display_data = apply_trend_override(
            ...
            current_product_override['type'],


            current_product_override.get('custom_rate', 0.0)
        ...
        override_label = {
            ...
            'last_growth': 'Continue Recent Growth', 


            'custom': f"Custom {current_product_override.get('custom_rate', 0)}% Monthly"


        }.get(current_product_override['type'], 'Custom')


        st.success(f"🔄 **Trend Override Applied**: {override_label} for {product}")
    ...
    display_product_forecast(display_data, product, chart_model, best_models_per_product, best_mapes_per_product)
    ...
    try:
        ...
        dfm_growth = (
            ...
            .rename("growth")              # series â†’ column name
        ...
        st.error(f"âŒ Growth chart failed for {product_mom}: {str(e)}")
    ...
    with col1:
        ...
        # Check if any trend overrides are active
        trend_overrides_active = False
        if 'trend_overrides' in st.session_state:
            for prod, override in st.session_state.trend_overrides.items():
                if override.get('type', 'none') != 'none':
                    trend_overrides_active = True
                    break
        
        # Add trend override option if any are active
        TREND_OVERRIDE_OPTION = "Best per Product (With Trend Overrides)"
        if trend_overrides_active:
            if TREND_OVERRIDE_OPTION not in download_options:
                download_options.insert(0, TREND_OVERRIDE_OPTION)
            default_choice = TREND_OVERRIDE_OPTION
        else:
        ...
        # Check if trend override option is selected
        if choice == "Best per Product (With Trend Overrides)":
            # Use Best per Product (Backtesting) as base and apply trend overrides
            base_choice = "Best per Product (Backtesting)" if "Best per Product (Backtesting)" in results else best_model
            if st.session_state.get('adjusted_forecast_results') is not None:
                download_data = st.session_state.adjusted_forecast_results[base_choice].copy()
            else:
                download_data = st.session_state.forecast_results[base_choice].copy()
            
            # Apply trend overrides to each product
            if 'trend_overrides' in st.session_state:
                for prod, override in st.session_state.trend_overrides.items():
                    if override.get('type', 'none') != 'none':
                        download_data = apply_trend_override(
                            download_data,
                            prod,
                            override['type'],
                            override.get('custom_rate', 0.0)
                        )
            
            # Show what overrides are included
            override_summary = []
            for prod, override in st.session_state.get('trend_overrides', {}).items():
                if override.get('type', 'none') != 'none':
                    override_type = override['type']
                    if override_type == 'custom':
                        override_summary.append(f"{prod}: {override.get('custom_rate', 0)}% monthly")
                    elif override_type == 'flat':
                        override_summary.append(f"{prod}: Flat trend")
                    elif override_type == 'last_growth':
                        override_summary.append(f"{prod}: Continue recent growth")
            if override_summary:
                st.success(f"🔄 Download includes trend overrides: {', '.join(override_summary)}")

        elif st.session_state.get('adjusted_forecast_results') is not None:
        ...
        with pd.ExcelWriter(buf2, engine="openpyxl") as writer:
            ...
            summary_rows = []
            summary_rows.append({"Category": "FORECAST SUMMARY", "Setting": "", "Value": "", "Details": ""})
            summary_rows.append({"Category": "", "Setting": "Generated", "Value": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "Details": ""})
            summary_rows.append({"Category": "", "Setting": "Model Selected", "Value": choice, "Details": ""})
            summary_rows.append({"Category": "", "Setting": "", "Value": "", "Details": ""})
            
            # Conservatism adjustment
            conservatism = st.session_state.get('forecast_conservatism_used', 100)
            summary_rows.append({"Category": "CONSERVATISM", "Setting": "", "Value": "", "Details": ""})
            if conservatism != 100:
                summary_rows.append({"Category": "", "Setting": "Conservatism Factor", "Value": f"{conservatism}%", "Details": "Applied to all forecast values"})
            else:
                summary_rows.append({"Category": "", "Setting": "Conservatism Factor", "Value": "100% (No adjustment)", "Details": ""})
            summary_rows.append({"Category": "", "Setting": "", "Value": "", "Details": ""})
            
            # Trend Overrides
            summary_rows.append({"Category": "TREND OVERRIDES", "Setting": "", "Value": "", "Details": ""})
            trend_overrides = st.session_state.get('trend_overrides', {})
            has_overrides = False
            for prod, override in trend_overrides.items():
                override_type = override.get('type', 'none')
                if override_type != 'none':
                    has_overrides = True
                    if override_type == 'custom':
                        rate = override.get('custom_rate', 0)
                        annualized = ((1 + rate/100)**12 - 1) * 100
                        summary_rows.append({
                            "Category": "", 
                            "Setting": prod, 
                            "Value": f"Custom {rate}% monthly",
                            "Details": f"≈ {annualized:.1f}% annualized (pure compounding)"
                        })
                    elif override_type == 'flat':
                        summary_rows.append({
                            "Category": "", 
                            "Setting": prod, 
                            "Value": "Flat Trend",
                            "Details": "Maintains last actual value"
                        })
                    elif override_type == 'last_growth':
                        summary_rows.append({
                            "Category": "", 
                            "Setting": prod, 
                            "Value": "Continue Recent Growth",
                            "Details": "Uses average of last 6 months growth"
                        })
            if not has_overrides:
                summary_rows.append({"Category": "", "Setting": "No trend overrides applied", "Value": "", "Details": ""})
            summary_rows.append({"Category": "", "Setting": "", "Value": "", "Details": ""})
            
            # Manual Adjustments
            summary_rows.append({"Category": "MANUAL ADJUSTMENTS", "Setting": "", "Value": "", "Details": ""})
            adj_type = st.session_state.get('adjustment_type')
            if adj_type == "manual" and st.session_state.get('product_adjustments_applied'):
                manual_adj = st.session_state.get('product_adjustments_applied', {})
                for prod, adj in manual_adj.items():
                    pct = adj.get('percentage', 0)
                    if pct != 0:
                        summary_rows.append({
                            "Category": "", 
                            "Setting": prod, 
                            "Value": f"{pct:+.1f}%",
                            "Details": "Applied to forecast period"
                        })
            else:
                summary_rows.append({"Category": "", "Setting": "No manual adjustments applied", "Value": "", "Details": ""})
            summary_rows.append({"Category": "", "Setting": "", "Value": "", "Details": ""})
            
            # Fiscal Year Growth Targets
            summary_rows.append({"Category": "FISCAL YEAR GROWTH TARGETS", "Setting": "", "Value": "", "Details": ""})
            if adj_type == "fiscal_year_growth" and st.session_state.get('fiscal_year_adjustments_applied'):
                fy_adj = st.session_state.get('fiscal_year_adjustments_applied', {})
                for prod, fy_targets in fy_adj.items():
                    for fy, target in fy_targets.items():
                        if target.get('enabled', False):
                            growth = target.get('target_yoy', target.get('growth_rate', 0))
                            summary_rows.append({
                                "Category": "", 
                                "Setting": f"{prod} - {fy}", 
                                "Value": f"{growth:+.1f}% YoY",
                                "Details": "Smoothed across fiscal year"
                            })
            else:
                summary_rows.append({"Category": "", "Setting": "No fiscal year targets applied", "Value": "", "Details": ""})
            summary_rows.append({"Category": "", "Setting": "", "Value": "", "Details": ""})
            
            # Renewals Overlay
            summary_rows.append({"Category": "RENEWALS OVERLAY", "Setting": "", "Value": "", "Details": ""})
            if st.session_state.get('yearly_renewals_applied', False):
                summary_rows.append({
                    "Category": "", 
                    "Setting": "Non-compliant Upfront RevRec", 
                    "Value": "Included",
                    "Details": "Shown as separate line items"
                })
            else:
                summary_rows.append({"Category": "", "Setting": "No renewals overlay applied", "Value": "", "Details": ""})
            
            # Create summary DataFrame and write to Excel
            summary_df = pd.DataFrame(summary_rows)
            summary_df.to_excel(writer, index=False, sheet_name="Forecast_Summary")
            
            # Format the summary sheet
            workbook = writer.book
            worksheet = writer.sheets["Forecast_Summary"]
        ...
        with st.expander("📊 **Model Performance Comparison**", expanded=False):
            ...
            if best_models_per_product:
                ...
                st.caption("📊 Models selected per product by backtesting WAPE (ties â†’ p75 WAPE â†’ MASE). Other metrics shown for diagnostics.")
                ...
                **How Product-by-Product Selection Works (Backtesting):**
                ...
                - Each model is walkâ€‘forward backtested per product (gap=0, h=6)


                - We compute WAPE, SMAPE, MASE, and RMSE; selection uses WAPE


                - Tieâ€‘breaks: p75 WAPE â†’ MASE â†’ recent worstâ€‘month error


                - Ineligible models (too few folds, MASE â‰¥ 1.0, unstable p95, or <5% better than Seasonalâ€‘Naive) are excluded
        ...
        # === ADVANCED VALIDATION (if available) â€” placed after Download Results ===
        ...
        with st.expander("â¬‡️ Download Results (Best per Product â€¢ Backtesting)", expanded=False):
        ...
        with st.expander("📍 **Technical Diagnostics**", expanded=False):
            ...
            if diagnostic_messages:
                ...
                categories = {
                    ...
                    "🔧 **Model Training**": ["✅", "âŒ", "MAPE", "Order", "Seasonal", "criterion"],
                    ...
                    "⚠️ **Warnings & Errors**": ["⚠️", "âŒ", "Error", "Failed", "skipping", "not available"]
                    ...
                    if messages:
                        ...
                        with st.expander(category, expanded=False):
                            ...
                            for msg in messages:
                                ...
                                elif "âŒ" in msg or "Error" in msg or "Failed" in msg:
                ...
                error_count = sum(1 for msg in diagnostic_messages if "âŒ" in msg)
                ...
                with col2:
                    ...
                    st.metric("âŒ Errors", error_count)
